...
pytest -k "test_lstm" -v
```

### Coverage Reports
```bash
# Generate coverage HTML report (opens in htmlcov/index.html)
...
### Test Organization
- **tests/test_dataset.py** — Feature engineering, scaling, DataLoader creation (target: 70%+ coverage)
- **tests/test_sentiment.py** — FinBERT scoring, lexicon fallback, sentiment aggregation (target: 70%+ coverage)
- **tests/test_model.py** — LSTM, Transformer, XGBoost initialization and forward passes (target: 70%+ coverage)
- **tests/test_optimize.py** — Portfolio optimization, Sharpe ratio, weight constraints (target: 70%+ coverage)

### Test Features
- **Fast execution**: All tests complete in <10 seconds using mocked APIs and small in-memory datasets
- **Mocked dependencies**: yfinance, transformers (FinBERT), cvxpy calls are mocked to avoid external dependencies
- **Edge cases**: Tests cover empty data, single samples, NaN values, constraint violations
- **Fixtures**: Reusable pytest fixtures for sample data (prices, sentiment articles, features)
...
# Run tests on file save (watch mode) — requires pytest-watch
